...
public class TicketServiceImpl implements TicketService {
    ...
    public ResponseEntity<?> getAllTickets() {
        try {
            List<Ticket> tickets = movieRepo.findAll().stream()
                    .flatMap(t -> t.getTickets().stream())
    ...
    public ResponseEntity<?> getTicketsUser(String userId) {
        ...
        List<Ticket> userTickets = movieRepo.findAll()
                .stream()
                .flatMap(movie -> movie.getTickets().stream())
                .filter(ticket -> Objects.equals(ticket.getUserId(), userId)) // Filter tickets by the provided user ID
                .collect(Collectors.toList());
